...
export default function DomainsListPage() {
  ...
  const loadAssets = useCallback(async () => {
    ...
    } catch { toast.error("도메인 목록을 불러올 수 없습니다"); }
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex items-center justify-between">
          <h1 className="text-3xl font-bold text-gray-900">도메인·SSL 관리</h1>
        ...
        <div className="mb-6 rounded-lg bg-white p-4 shadow-sm">
          <div className="mb-4">
            <input type="text" placeholder="도메인명 검색..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" />
          </div>
          ...
          <table className="w-full">
            <thead className="border-b bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold">공급업체</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold">담당자</th>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={7} className="px-6 py-8 text-center text-gray-500">도메인을 찾을 수 없습니다</td></tr>
              ) : (
                assets.map((a) => (
                  <tr key={a.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => router.push(`/domains/${a.id}`)} className="rounded p-1 hover:bg-gray-200" title="상세"><Eye className="h-4 w-4" /></button>
                        {user && (<>
                          <button onClick={() => router.push(`/domains/${a.id}/edit`)} className="rounded p-1 hover:bg-gray-200" title="수정"><Edit className="h-4 w-4" /></button>
        ...
        <div className="mt-4 text-sm text-gray-600">총 {total}개 도메인·SSL</div>
      </div>
    </div>
  );
}
