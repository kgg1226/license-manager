...
export default function CloudDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!res.ok) { toast.error("자산을 찾을 수 없습니다"); router.push("/cloud"); return; }
  ...
  if (!asset) return <div className="min-h-screen bg-gray-50 p-6"><div className="mx-auto max-w-4xl"><p className="text-center text-red-600">자산을 찾을 수 없습니다</p><div className="mt-4 text-center"><Link href="/cloud" className="text-blue-600 hover:underline">목록으로</Link></div></div></div>;
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div className="rounded-lg bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-2xl font-bold text-gray-900">{asset.cloudDetail?.platform || "—"}</p>
          </div>
          <div className="rounded-lg bg-white p-6 shadow-sm">
        ...
        {user && (
          <div className="flex gap-3">
            <Link href={`/cloud/${asset.id}/edit`} className="flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700"><Edit className="h-4 w-4" />수정</Link>
